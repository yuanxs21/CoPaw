...
- [v1.1.1] 新機能：OpenRouter と OpenCode モデルプロバイダー；モデル追加時のモデル ID オートコンプリート、全プロバイダーでモデル検出をデフォルト有効化；`list_agents` と `chat_with_agent` エージェント協調ツールを内蔵；Matrix チャネルを E2EE・メンション対応・メッセージ履歴付きで全面刷新；Feishu 引用メッセージ対応；DingTalk QR 認証；シェルコマンドガードルールの拡張；RESTful API チュートリアル。
- [v1.1.1] 変更：DingTalk をアリババクラウド公式 SDK へ移行；マルチモーダル探査の統一化；モデル切替時のメディアメモリ保持；ブラウザのデフォルト起動方式をマネージド CDP に変更；モデル管理画面の刷新；エージェント設定のタブ化；スキル選択の改善。
- [v1.1.1] 修正：QQ チャネルの `stop()` が 8 秒ブロックしなくなりました；Windows でのローカルモデルダウンロード失敗を修正；vLLM の `tool_choice` 互換性を改善。
- [v1.1.1] 新規コントリビューター：@maplefeng-a、@lhpqaq、@jilin6627-spec の皆さん、ありがとうございます。
