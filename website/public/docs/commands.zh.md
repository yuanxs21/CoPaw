...
多步骤任务使用**计划模式**。模型不会在每条消息上被提示创建计划；需要显式发起。

| 形式           | 行为                                                                                |
| -------------- | ----------------------------------------------------------------------------------- |
| `/plan`        | 若未开启则开启计划模式，并显示当前是否有计划及子任务（或提示使用 `/plan <描述>`）。 |
| `/plan <描述>` | 作为普通聊天处理（去掉前缀）：智能体收到明确的「为该任务创建计划」请求。            |
...
**兼容性说明：** 以往依赖模型在「无计划」时每次看到默认提示的行为已改变；请使用 `/plan` 或控制台「计划」面板开始规划。
